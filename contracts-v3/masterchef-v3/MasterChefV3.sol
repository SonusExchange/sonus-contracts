...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    modifier onlyOwnerOrMasterchefV2() {
        if (msg.sender != owner() && msg.sender != masterchefV2) revert NotOwnerOrOperator();
        _;
    }

    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(INonfungiblePositionManager _nonfungiblePositionManager, address _WETH, address initialOwner, address _rewardsAddress) Ownable() {
    ...
    function setMasterchefV2(address _masterchefV2) external onlyOwner {
    ...
    function add(uint256 _allocPoint, IPancakeV3Pool _v3Pool, bool _withUpdate) external onlyOwnerOrMasterchefV2 {
    ...
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) external onlyOwnerOrMasterchefV2 onlyValidPid(_pid) {
    ...
    /// @notice Harvest multiple NFTs.
    /// @param tokenIds Token Ids of NFT.
    /// @param _to Address to.
    function bulkHarvest(uint256[] calldata tokenIds, address _to) external nonReentrant returns (uint256[] memory rewards) {
        rewards = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            UserPositionInfo storage positionInfo = userPositionInfos[tokenIds[i]];
            if (positionInfo.user != msg.sender) revert NotOwner();
            if (positionInfo.liquidity == 0 && positionInfo.reward == 0) revert NoLiquidity();
            uint256 reward = harvestOperation(positionInfo, tokenIds[i], _to);
            rewards[i] = reward;
        }
    }
    ...
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyOwnerOrMasterchefV2 {
        if (_withUpdate) massUpdatePools();
    ...
    function setGlobalCakePerSecond(uint256 _globalCakePerSecond) external onlyOwnerOrMasterchefV2 {
    ...
    function _safeTransfer(address _to, uint256 _amount, uint256 _pid) internal {
        if (_amount > 0) {
                require(
                    IERC20Mintable(rewardsAddress).mint(address(_to), _amount),
