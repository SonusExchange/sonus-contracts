...
contract SonusRouter is IUniswapV2Router02 {
    ...
    function _pairFor(address _factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f4fe426305566a0a6b0ace5977cdc55cdff58b8e37f568dc31f3069618200038' // init code hash
            ))));
    }

    // fetches and sorts the reserves for a pair
    function _getReserves(address _factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = _sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IUniswapV2Pair(_pairFor(_factory, tokenA, tokenB)).getReserves();
    ...
    function _getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint swapFee) internal pure returns (uint amountOut) { // fix
    ...
    function _getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint swapFee) internal pure returns (uint amountIn) { // fix
    ...
    function _getAmountsOut(address _factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'UniswapV2Library: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = _getReserves(_factory, path[i], path[i + 1]);
            address pair = _pairFor(factory, path[i], path[i + 1]);
            uint swapFee = IUniswapV2Pair(pair).swapFee();
            amounts[i + 1] = _getAmountOut(amounts[i], reserveIn, reserveOut, swapFee);
        }
    }

    // performs chained getAmountIn calculations on any number of pairs
    function _getAmountsIn(address _factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'UniswapV2Library: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[amounts.length - 1] = amountOut;
        for (uint i = path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut) = _getReserves(_factory, path[i - 1], path[i]);
            address pair = _pairFor(factory, path[i], path[i + 1]);
            uint swapFee = IUniswapV2Pair(pair).swapFee();
            amounts[i - 1] = _getAmountIn(amounts[i], reserveIn, reserveOut, swapFee);
        }
    }
    /************************* NEW UniswapV2Library *************************/
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = _sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(_pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            uint swapFee = pair.swapFee();
            amountOutput = _getAmountOut(amountInput, reserveInput, reserveOutput, swapFee);
            }
    ...
    {
        uint swapFee = IUniswapV2Pair(_pairFor(factory, tokenA, tokenB)).swapFee();
        return _getAmountOut(amountIn, reserveIn, reserveOut, swapFee);
    }
    ...
    {
        uint swapFee = IUniswapV2Pair(_pairFor(factory, tokenA, tokenB)).swapFee();
        return _getAmountIn(amountOut, reserveIn, reserveOut, swapFee);
    }
