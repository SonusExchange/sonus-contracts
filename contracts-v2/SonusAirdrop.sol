...
// File: contracts/interfaces/ISonus.sol
...
/// @title SonusAirdrop
/// @notice Claims SONUS for members of a merkle tree
contract SonusAirdrop {
    /// @notice max lock period 104 weeeks
    uint256 public constant LOCK = 104 * 7 * 86400;
    ...
    /// @notice Mapping from boost level to veSONUS token amount
    ...
    /// @param _claimAmount amount of veSONUS to claim
    ...
    function claim(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(_to, _claimAmount));
        ...
        require(sonus.balanceOf(address(this)) >= claimAmount, "All tokens were already claimed");

        // Set address to claimed
        hasClaimed[_to] = true;
        sonus.approve(address(ve), claimAmount);
        // Claim veSONUSs for address
    ...
    function withdrawSONUS(address _recipient) external onlyOwner {
        require(block.timestamp > startTime + duration * 86400, "Airdrop is not finished yet");
        uint256 remaining = sonus.balanceOf(address(this));
        require(remaining > 0, "No remaining tokens");
        sonus.transfer(_recipient, remaining);
