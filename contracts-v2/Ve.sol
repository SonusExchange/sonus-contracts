...
contract Ve is IERC721, IERC721Metadata, IVe, Reentrancy {
    ...
    uint internal constant MAX_TIME = 104 * 7 * 86400; // 104 weeks
    int128 internal constant I_MAX_TIME = 104 * 7 * 86400; // 104 weeks
    ...
    string public constant override name = "veSONUS";
    string public constant override symbol = "veSONUS";
    ...
    function setMinterContract(address _minterContract) external { // onlyVoter
        require(msg.sender == voterContract);
        minterContract = _minterContract;
    }

    function setVoterContract(address _voterContract) external { // onlyVoter
        require(msg.sender == voterContract);
        voterContract = _voterContract;
    }
    ...
    function _transferFrom(
        ...
        require(!IVoter(voterContract).nftCantVote(_tokenId), "NFT CANT VOTE");
    ...
    function _depositFor(
        ...
        if (_value != 0 && depositType != DepositType.MERGE_TYPE && depositType != DepositType.SPLIT_TYPE) {
    ...
    function voting(uint _tokenId) external override {
        require(msg.sender == voterContract, "!voter");
        voted[_tokenId] = true;
    }
    ...
    function merge(uint _from, uint _to) external {
        require(
            !IVoter(voterContract).nftCantVote(_from) && !IVoter(voterContract).nftCantVote(_to),
            "!ACTIVE"
        );
    ...
    function split(uint256 _from, uint256 _amount) external returns (uint256 newTokenId) {
        require(!IVoter(voterContract).nftCantVote(_from), "NFT CANT VOTE");
        require(_amount > 0);
        require(!voted[_from], "reset votes");
        require(_isApprovedOrOwner(msg.sender, _from));

        LockedBalance memory initialLocked = locked[_from];
        uint256 oldAmount = uint256(int256(initialLocked.amount));

        require(initialLocked.end > block.timestamp, "Cannot add to an expired lock. Withdraw");
        ...
        _depositFor(tokenId, _amount, initialLocked.end, newLocked, DepositType.SPLIT_TYPE);
        newTokenId = tokenId;
    }
    ...
    function _createLock(
        ...
        require(
            ...
            require(
                ...
                "Voting lock can be 104 weeks max"
            );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "lock #',
                        _toString(_tokenId),
                        '", "description": "Sonus locks, can be used to vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,',
